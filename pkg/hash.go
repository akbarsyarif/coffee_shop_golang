...
func (h *HashConfig) ComparePassword(password, hashedPassword string) (bool, error) {
	salt, hash, err := h.decodeHash(hashedPassword)
	if err != nil {
		return false, err
	}

	//generate hash dari password yang datang
	bodyPwHash := argon2.IDKey([]byte(password), salt, h.Time, h.Memory, h.Threads, h.KeyLen)
	if subtle.ConstantTimeCompare(bodyPwHash, hash) == 0 {
		return false, nil
	}
	return true, nil
}
...
func (h *HashConfig) decodeHash(hashedPassword string) (salt []byte, hash []byte, err error) {
	...
	if len(values) != 6 {
		return nil, nil, errors.New("Invalid Hashed Password")
		}
